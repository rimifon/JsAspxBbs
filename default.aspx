	...
	return apidoc({
		...
		,forum: function() {
			...
			var showPage = function(x) {
				...
				for(var i = 1; i <= page; i++) arr.push(i.toString().link("topic/" + x.topicid + "/" + i));
		...
		,weibo: {
			...
			,topiclist: function() {
				...
				var topics = query.orderby("topicid desc").select("topicid").limit(0, 11).astable("a").
					...
					select("a.*, b.title, b.pv, b.replynum, b.posttime, c.nick, c.icon, d.nick as forumname, e.replyid, e.message").query(par);
