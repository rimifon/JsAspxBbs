<!-- #include file="AspxLib/inc.aspx" --><%
function boot(route) {
	if(route[1] == "ClearLogs") return clearLogs();
	sys.debug = false;
	sys.name = "网站访问监控";
...
function web(data) {
	var forEach = function(func) { for(var i = 0; i < this.length; i++) func(this[i]); };
	var last = data.last || new Array, slow = data.slow || new Array, logs = data.logs || new Array;
	var qps = last.length > 1 ? (last.length * 1000 / (new Date(last[0][4]) - new Date(last[ last.length - 1 ][4]))).toFixed(1) - 0 : 1;
...
<div class="tab">
	...
	<div class="page">
		<table border="1" cellpadding="6">
			...
			</tr><% forEach.call(last, function(x) { %>
			<tr>
				<td><%= x[0] %></td>
				<td><%= x[1] %></td>
				<td><%= x[2] %></td>
	...
	<div class="page" hidden="true">
		<table border="1" cellpadding="6">
			...
			</tr><% forEach.call(slow, function(x) { %>
			<tr>
				<td><%= x[2] %></td>
				<td><%= x[5] %>ms</td>
				<td><%= x[0] %></td>
	...
	<div class="page" hidden="true">
		<table border="1" cellpadding="6">
			...
			<tr>
				<th>路由</th><th>耗时</th><th>方式</th><th>路径</th><th>发生时间</th>
			</tr><% forEach.call(logs, function(x) { %>
			<tr>
				<td><%= x.info[2] %></td>
				<td><%= x.info[4] %>ms</td>
				<td><%= x.info[0] %></td>
				<td>
					<%= x.info[1] %><% forEach.call(x.rows, function(y) { %>
					<br /><%= y[0] %>[<%= y[1] %>ms]<% }); %>
				</td>
				<td><p><%= x.info[3] %></p></td>
			</tr><% }); %>
...
(function() {
	...
	document.querySelector("table caption p tt").onclick = function() {
		...
		xhr.open("POST", "<%= env("URL") %>/ClearLogs", true);
